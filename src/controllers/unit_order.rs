            ...
            state.events.iter().find_map(|event| {
                match event {
                    Event::KeyPressed { code, .. } => {
                        ...
                        } else if *code == order_config.build_mine && unit.unit_type == UnitType::Settler {
                            Some(vec![HexEvent::new_from_unit("BUILD_MINE", unit)])
                        } else if *code == order_config.transform && unit.unit_type == UnitType::Settler {
                            Some(vec![HexEvent::new_from_unit("UNIT_TRANSFORM_HEX", unit)])
                        } else {
                            None
                        }
                    },
