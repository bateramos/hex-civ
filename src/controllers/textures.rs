...
fn load_texture<'a>(texture: &'a Texture, x: i32, y: i32, scale_x: f32, scale_y: f32) -> Sprite {
    load_texture_with_rect(texture, scale_x, scale_y, IntRect::new(x * SPRITE_X_PADDING , y * SPRITE_Y_PADDING, 32, 50))
}

fn load_texture_with_rect<'a>(texture: &'a Texture, scale_x: f32, scale_y: f32, rect: IntRect) -> Sprite {
    let mut sprite = Sprite::with_texture_and_rect(&texture, &rect);
